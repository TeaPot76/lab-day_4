...
class TestFriends < MiniTest::Test
  ...
  def test_getting_fav_show
    result = get_fav_show(@person2)
...
def test_lend_money__lender_money()

  lend_money(@person5, @person1, 10)
  result_lender_money = @person5[:monies]
  assert_equal(90, result_lender_money)
end

def test_lend_money__borrower_money()

  lend_money(@person5, @person1, 10)
  result_borrower_money = @person1[:monies]
  assert_equal(11, result_borrower_money)
end

  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/snack arrays together)
def test_fav_food_all
  result = fav_food_all(@people)
